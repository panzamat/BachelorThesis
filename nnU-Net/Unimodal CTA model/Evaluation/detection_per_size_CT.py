...
if __name__ == "__main__":
    ...
    with tqdm(total=len(folds), desc=f"Processing {dataset}") as pbar:
        for i in folds:
            ...
            component_wise_csv_file_name = f"component_{dataset}_f{i}_detection_per_size_CT.csv"
    ...
    all_metrics_df.to_csv(f"all_aggregated_{dataset}_detection_per_size_CT.csv", index=False)
