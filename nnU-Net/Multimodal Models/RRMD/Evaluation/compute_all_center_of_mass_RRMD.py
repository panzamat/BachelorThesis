...
def summarize_metrics(all_metrics_df, dataset):
    ...
    metrics_summary['formatted_result'].to_csv(f"formatted_summary_{dataset}_center_of_mass_after_filtering_064.csv", header=True)
...
def summarize_metrics_CI(all_metrics_df, dataset):
    ...
    metrics_summary['formatted_result'].to_csv(f"formatted_summary_{dataset}_95CI_com_064.csv", header=True)
    print(f"Formatted summary saved to formatted_summary_{dataset}_95CI_com_064.csv")
...
if __name__ == "__main__":
    ...
    with tqdm(total=len(folds), desc=f"Processing {dataset}") as pbar:
        for i in folds:
            pred_files_path = f'/cfs/earth/scratch/icls/shared/comp-health-lab/data/aneu/nnUNet_Frame/Mattia_BSc_Thesis/multimodal_data/nnUNet_data/nnUNet_results/Dataset064_IA/postprocessed/postprocessed_f{i}'
            gt_files_path = f'/cfs/earth/scratch/icls/shared/comp-health-lab/data/aneu/nnUNet_Frame/Mattia_BSc_Thesis/multimodal_data/nnUNet_data/nnUNet_raw/Dataset064_IA/labelsTs'
            ...
            # Output
            aggregated_csv_file_name = f"aggregated_{dataset}_f{i}_center_of_mass_after_filtering_064.csv"
            component_csv_file_name = f"component_{dataset}_f{i}_center_of_mass_after_filtering_064.csv"
    ...
    all_metrics_df.to_csv(f"all_aggregated_{dataset}_center_of_mass_after_filtering_064.csv", index=False)
