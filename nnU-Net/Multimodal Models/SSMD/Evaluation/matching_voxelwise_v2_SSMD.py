...
def summarize_metrics_CI(all_metrics_df, dataset):
    ...
    metrics_summary['formatted_result'].to_csv(f"formatted_summary_{dataset}_95CI_voxelwise_CTMR_nnUNet_spacing.csv", header=True)
    print(f"Formatted summary saved to formatted_summary_{dataset}_95CI_voxelwise_CTMR_nnUNet_spacing.csv")
...
if __name__ == "__main__":
    ...
    with tqdm(total=len(folds), desc=f"Processing {dataset}") as pbar:
        for i in folds:
            ...
            # Output paths
            aggregated_csv_file_name = f"aggregated_{dataset}_f{i}_voxelwise_CT_CTMR_nnUNet_spacing.csv"
            component_wise_csv_file_name = f"voxelwise_{dataset}_f{i}_CT_CTMR_nnUNet_spacing.csv"
    ...
    all_metrics_df.to_csv(f"all_aggregated_metrics_{dataset}_voxelwise_CI_CTMR_nnUNet_spacing.csv", index=False)
