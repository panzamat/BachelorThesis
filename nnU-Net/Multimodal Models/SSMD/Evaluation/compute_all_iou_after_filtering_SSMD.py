...
from compute_all_center_of_mass_CTMR import aggregate_metrics
import os
import pandas as pd
import numpy as np
np.bool = np.bool_
...
def summarize_metrics(all_metrics_df, dataset):
    ...
    metrics_summary['formatted_result'].to_csv(f"formatted_summary_{dataset}_iou_after_filtering_CTMR_nnUNet_spacing.csv", header=True)
...
def summarize_metrics_CI(all_metrics_df, dataset):
    ...
    metrics_summary['formatted_result'].to_csv(f"formatted_summary_{dataset}_95CI_iou_CTMR_nnUNet_spacing.csv", header=True)
    print(f"Formatted summary saved to formatted_summary_{dataset}_95CI_iou_CTMR_nnUNet_spacing.csv")
...
if __name__ == "__main__":
    ...
    with tqdm(total=len(folds), desc=f"Processing {dataset}") as pbar:
        for i in folds:
            ...
            # Output-Dateien
            aggregated_csv_file_name = f"aggregated_{dataset}_f{i}_iou_after_filtering_CTMR_nnUNet_spacing.csv"
            component_csv_file_name = f"component_{dataset}_f{i}_iou_after_filtering_CTMR_nnUNet_spacing.csv"
    ...
    all_metrics_df.to_csv(f"all_aggregated_{dataset}_iou_after_filtering_CTMR_nnUNet_spacing.csv", index=False)
