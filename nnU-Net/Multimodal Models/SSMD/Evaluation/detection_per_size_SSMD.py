...
from detection_per_size_summary_CTMR_personal_spacing import compute_detection_per_size_ci
...
def summarize_metrics(all_metrics_df, dataset):
    ...
    formatted_summary.to_csv(f"formatted_summary_{dataset}_detection_per_size_CTMR_nnUNet_spacing.csv", header=True)
    print(f"Formatted summary saved to formatted_summary_{dataset}_detection_per_size_CTMR_nnUNet_spacing.csv")
...
def summarize_metrics_CI(all_metrics_df, dataset):
    ...
    summary_df.to_csv(f"formatted_summary_{dataset}_95CI_detection_per_size_CTMR_nnUNet_spacing.csv", index=False)
    print(f"📄 Saved summary: formatted_summary_{dataset}_95CI_detection_per_size_CTMR_nnUNet_spacing.csv")
...
if __name__ == "__main__":
    ...
    with tqdm(total=len(folds), desc=f"Processing {dataset}") as pbar:
        for i in folds:
            ...
            component_wise_csv_file_name = f"component_{dataset}_f{i}_detection_per_size_CTMR_nnUNet_spacing.csv"
    ...
    all_metrics_df.to_csv(f"all_aggregated_{dataset}_detection_per_size_CTMR_nnUNet_spacing.csv", index=False)
